        ...
        <div className="bg-slate-50/70 dark:bg-slate-955/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-lg max-w-sm mx-auto font-mono text-sm text-slate-800 dark:text-slate-200">
          <div className="text-center space-y-1">
            ...
            {coopSettings?.cooperative_phone && (
              <p className="text-xs text-slate-500 dark:text-slate-400">Telp: {coopSettings.cooperative_phone}</p>
            )}
          ...
          <div className="space-y-1 text-xs font-semibold">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-slate-905 dark:text-slate-100">
                {selectedSale.member
                  ? selectedSale.member.Siswa?.nama_siswa ||
                    selectedSale.member.Guru?.nama_guru ||
                    selectedSale.member.User?.full_name ||
                    'Anggota'
                  : 'Umum'}{' '}
                {selectedSale.member?.memberNo ? `(${selectedSale.member.memberNo})` : ''}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-1.5 text-xs font-semibold">
            ...
            <div className="flex justify-between font-extrabold text-slate-955 dark:text-slate-55 text-sm pt-1">
              <span>TOTAL</span>
            ...
            {selectedSale.paymentMethod === 'CASH' && (
              <>
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="text-slate-700 dark:text-slate-300">Rp {Number(selectedSale.cashReceived || 0).toLocaleString('id-ID')}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>Kembalian:</span>
                  <span className="text-slate-700 dark:text-slate-300">Rp {Number(selectedSale.change || 0).toLocaleString('id-ID')}</span>
                </div>
              </>
            )}
          </div>
