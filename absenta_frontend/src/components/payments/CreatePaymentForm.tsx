...
import type { CreatePaymentFormProps, PaymentGateway, PaymentMethod, PaymentStatusData, GatewayConfig } from '../../types/paymentGateway.d';
...
export default function CreatePaymentForm({
  ...
  const [tripayChannels, setTripayChannels] = useState<Array<{
    code: string;
    name: string;
    group?: string;
  }>>([]);
  ...
  return <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {paymentResult ? <div className="space-y-4">
          ...
          {paymentResult.qrString && <div className="text-center">
              <h4 className="font-medium mb-2">Scan QR Code untuk Pembayaran</h4>
              <div className="inline-block p-4 bg-white border-2 border-gray-200 rounded-lg">
                <img src={paymentResult.qrString} alt="QR Code Pembayaran" className="w-48 h-48 mx-auto" />
              </div>
              {paymentResult.expiresAt && <p className="text-sm text-gray-600 mt-2">
                  Berlaku hingga: {new Date(paymentResult.expiresAt).toLocaleString('id-ID')}
                </p>}
            </div>}

          {paymentResult.paymentUrl && !paymentResult.qrString && <div className="text-center">
              <h4 className="font-medium mb-2">Link Pembayaran</h4>
              <a href={paymentResult.paymentUrl} target="_blank" rel="noopener noreferrer" className="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
          ...
          <div>
            ...
            <SearchableSelect value={gateway} onValueChange={val => setGateway(val as PaymentGateway)} options={GATEWAY_OPTIONS.map(option => ({
          label: `${option.label} ${!isGatewaySupported(option.value) ? '(Tidak Didukung)' : ''}`,
          value: option.value,
          disabled: !isGatewaySupported(option.value)
        }))} placeholder="Pilih Gateway" searchPlaceholder="Cari gateway..." triggerClassName="w-full" />
          </div>
          ...
          {gateway !== 'TRIPAY' && <div>
              ...
              <SearchableSelect value={paymentMethod} onValueChange={val => setPaymentMethod(val as PaymentMethod)} options={METHOD_OPTIONS.map(option => ({
          label: `${option.label} ${!isMethodSupported(option.value) ? '(Tidak Didukung)' : ''}`,
          value: option.value,
          disabled: !isMethodSupported(option.value)
        }))} placeholder="Pilih Metode Pembayaran" searchPlaceholder="Cari metode..." triggerClassName="w-full" />
            </div>}
          ...
          {gateway === 'TRIPAY' && <div>
              ...
              <SearchableSelect value={selectedChannelCode} onValueChange={val => setSelectedChannelCode(val)} options={[{
          label: '-- Pilih Channel --',
          value: ''
        }, ...filteredTripayChannels.map(ch => ({
          label: ch.name || ch.code,
          value: ch.code
        }))]} placeholder="Pilih Channel" searchPlaceholder="Cari channel..." triggerClassName="w-full" />
              {tripayChannelsError && <p className="text-xs text-red-600 mt-1">
                  {tripayChannelsError}
                </p>}
              {!tripayChannelsError && filteredTripayChannels.length === 0 && <p className="text-xs text-gray-500 mt-1">
          ...
          <div>
            ...
            <input type="url" id="returnUrl" value={returnUrl} onChange={e => setReturnUrl(e.target.value)} placeholder="https://example.com/success" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          ...
          <div>
            ...
            <input type="url" id="cancelUrl" value={cancelUrl} onChange={e => setCancelUrl(e.target.value)} placeholder="https://example.com/cancel" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>

          {/* Submit Button */}
          <div className="flex justify-end pt-4 border-t mt-6">
            <Button type="submit" disabled={isSubmitting || !!tripayChannelsError}>
