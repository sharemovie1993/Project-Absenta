...
export const JadwalTemplateList: React.FC<{ kelasId?: string }> = ({ kelasId }) => {
  ...
  // Edit State
  const [editingItem, setEditingItem] = useState<JadwalTemplate | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const fetchJadwal = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) {
          LogService.error('Failed to fetch jadwal template', err);
        }
      } finally {
        if (!controller.signal.aborted) {
          setLoading(false);
  ...
  const executeDelete = useCallback(async () => {
    ...
    try {
      setIsDeleting(true);
      await deleteJadwalTemplate(deletingId);
                    ...
                    <div key={`new-${day}-${idx}`} className="p-4 rounded-2xl border border-indigo-200 dark:border-indigo-900/50 bg-indigo-50/30 dark:bg-indigo-950/10 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <Label className="text-[10px] font-black uppercase tracking-wider text-slate-500">Mulai</Label>
                            <Input type="time" className="rounded-xl h-10" value={row.jam_mulai} onChange={(e) => updatePendingField(day, idx, 'jam_mulai', e.target.value)} />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-[10px] font-black uppercase tracking-wider text-slate-500">Selesai</Label>
                            <Input type="time" className="rounded-xl h-10" value={row.jam_selesai} onChange={(e) => updatePendingField(day, idx, 'jam_selesai', e.target.value)} />
                          </div>
                        </div>
                      </div>
                      {tipe.startsWith('KBM') && (
