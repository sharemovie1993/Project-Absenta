...
import { Download, FileText, Plus, Loader } from 'lucide-react';
import { 
  Button, 
  Modal, 
  ModalFooter, 
  ...
  <Modal isOpen={isOpen} onClose={onClose} title="Upload Dokumen" description="Unggah dokumen baru ke Document Center" size="md">
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Judul</label>
        <Input className="w-full rounded-xl" placeholder="Contoh: Panduan Penggunaan" {...register('title')} error={errors.title?.message} />
      </div>
      <div className="space-y-1">
      ...
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Deskripsi</label>
        <Textarea rows={3} placeholder="Opsional" className="w-full rounded-xl" {...register('description')} />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">File</label>
        <div className="relative group">
            <input type="file" className="w-full text-xs" onChange={(e) => setValue('file', e.target.files?.[0], { shouldValidate: true })} />
...
export const MouModal: React.FC<MouModalProps> = ({
  ...
  <Modal isOpen={isOpen} onClose={onClose} title="Generate MoU" description="Generate MoU dalam bentuk PDF dan simpan ke Document Center" size="md">
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Judul</label>
        <Input className="w-full rounded-xl" placeholder="Opsional" {...register('title')} error={errors.title?.message} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Nomor</label>
          <Input className="w-full rounded-xl" placeholder="Opsional" {...register('nomor')} error={errors.nomor?.message} />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Tanggal</label>
          <Input type="date" className="w-full rounded-xl" {...register('tanggal')} error={errors.tanggal?.message} />
        </div>
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Pihak Kedua (Nama)</label>
        <Input className="w-full rounded-xl" placeholder="Opsional" {...register('pihak_kedua_nama')} error={errors.pihak_kedua_nama?.message} />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Pihak Kedua (Alamat)</label>
        <Textarea rows={2} placeholder="Opsional" className="w-full rounded-xl" {...register('pihak_kedua_alamat')} />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Deskripsi</label>
        <Textarea rows={3} placeholder="Opsional" className="w-full rounded-xl" {...register('description')} />
...
export const EditMetadataModal: React.FC<EditMetadataModalProps> = ({
  ...
  <Modal isOpen={isOpen} onClose={onClose} title="Edit Metadata" description={target ? `Perbarui metadata untuk "${target.title}"` : 'Perbarui metadata dokumen'} size="md">
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Judul</label>
        <Input className="w-full rounded-xl" placeholder="Judul dokumen" {...register('title')} error={errors.title?.message} />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Kategori</label>
        <SearchableSelect
