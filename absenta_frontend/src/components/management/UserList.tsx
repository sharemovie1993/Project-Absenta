...
import { Table, Button, Input, Modal, Badge, Loader, Card, CardHeader, CardTitle, CardContent, Checkbox } from '../ui';
...
const UserList: React.FC<UserListProps> = ({
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      const response = await getUsers(currentPage, itemsPerPage, debouncedFilters.search, tenantId, {
  ...
  const getRoleBadge = (roleName: string) => {
    ...
    return <Badge variant="outline" className={roleColors[roleName] || 'bg-gray-100 text-gray-800'}>
        {roleName}
      </Badge>;
  };
  ...
  return <div className="flex flex-col">
      ...
      <div className="flex flex-wrap items-center px-4 py-2 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-slate-900 overflow-x-auto gap-2">
          {canManage && onAdd && <div onClick={onAdd} className="flex items-center cursor-pointer text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors select-none py-1">
          ...
          try {
            ...
            if (failed.length > 0) {
              ...
              toast(`Berhasil: ${succeeded.length}, Gagal: ${failed.length}`, {
                icon: '⚠️'
              });
            } else {
          ...
          <Button variant="ghost" size="sm" className="h-7 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-800 px-2" onClick={() => loadUsers()} aria-label="Refresh Data" disabled={loading}>
      ...
      <div className="px-1 border-b border-gray-50 dark:border-gray-800">
        <UserFilter filters={filters} onFiltersChange={handleFiltersChange} onReset={handleResetFilters} />
      </div>

      {/* Users Table */}
      <div className="bg-white dark:bg-slate-950 overflow-hidden">
        <Table compact={true} columns={[...(canManage ? [{
        key: '__select',
        label: <Checkbox checked={allVisibleSelected} onCheckedChange={checked => {
          if (checked) {
            const next = new Set<string>(selectedIds);
        ...
        className: 'w-16',
        render: (_: any, row: User) => <Checkbox checked={selectedIds.has(row.id)} onCheckedChange={checked => {
          const next = new Set<string>(selectedIds);
        ...
        key: 'email',
        label: 'Email',
        render: (value, row) => <div className="font-medium text-gray-900 dark:text-gray-100">
                  {value}
                </div>
      }, {
        key: 'full_name',
        label: 'Nama Lengkap',
        render: (value, row) => <div className="text-gray-900 dark:text-gray-100">
                  {value}
                </div>
      }, {
        key: 'role',
        ...
        key: 'tenant',
        label: 'Tenant',
        render: (value: unknown, row: User) => <div className="text-gray-600 dark:text-gray-400">
        ...
        render: (value, row) => <div className="flex items-center gap-2">
                  {onView && <Button variant="outline" size="sm" onClick={() => onView(row)} aria-label={`Lihat Detail ${row.full_name}`} className="flex items-center">
                      <Eye className="h-4 w-4" />
                    </Button>}
                  
                  {onEdit && canEditUser(row) && <Button variant="outline" size="sm" onClick={() => onEdit(row)} aria-label={`Edit Data ${row.full_name}`} className="flex items-center">
                      <Edit className="h-4 w-4" />
                    </Button>}
                  {canResetPassword(row) && <Button variant="outline" size="sm" onClick={() => {
            setSelectedUser(row);
            setNewPassword('');
            setConfirmPassword('');
            setResetModalOpen(true);
          }} aria-label={`Reset Password ${row.full_name}`} className="flex items-center">
                      <KeyRound className="h-4 w-4" />
                    </Button>}
                  
                  {canDeleteUser(row) && <Button variant="outline" size="sm" onClick={() => handleDeleteClick(row)} aria-label={`Hapus Pengguna ${row.full_name}`} className="flex items-center text-red-600 hover:text-red-700 hover:bg-red-50">
                      <Trash2 className="h-4 w-4" />
                    </Button>}
                </div>
      }]} data={users} loading={loading} emptyMessage={filters.search || filters.role || filters.status || filters.tenant ? 'Tidak ada pengguna yang sesuai dengan filter.' : 'Belum ada pengguna yang terdaftar.'} pagination={{
        currentPage,
        totalPages,
        totalItems,
        itemsPerPage,
      ...
      <Modal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Konfirmasi Hapus Pengguna" size="sm">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button variant="danger" onClick={handleDeleteConfirm} disabled={deleting} className="flex items-center">
      ...
      <Modal isOpen={resetModalOpen} onClose={() => setResetModalOpen(false)} title="Reset Password Pengguna" size="sm">
        <div className="space-y-4">
      ...
      <Modal isOpen={bulkErrorModalOpen} onClose={() => {
      ...
      setBulkErrorDetails([]);
        ...
        <div className="space-y-3">
          ...
          <div className="border rounded p-3 max-h-64 overflow-auto">
            {bulkErrorDetails.map(e => <div key={e.id} className="text-sm py-1 border-b last:border-b-0 border-slate-200 dark:border-slate-700">
                {e.name}: {e.message}
              </div>)}
          </div>
