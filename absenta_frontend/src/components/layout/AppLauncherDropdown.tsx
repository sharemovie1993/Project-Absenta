      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 top-12 z-50 w-[340px] sm:w-[380px] rounded-3xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-2xl overflow-hidden p-4"
          >
            {/* Header / Home Portal Shortcut */}
            <div className="flex items-center justify-between pb-3 mb-2 border-b border-slate-100 dark:border-slate-800/80 px-1">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-950/30 px-2.5 py-1 rounded-full transition-colors"
              >
            ...
            {/* 3-Column Apps Grid */}
            <div className="grid grid-cols-3 gap-2 max-h-[380px] overflow-y-auto overscroll-contain p-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
              {visibleApps.map((app) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "group relative flex flex-col items-center justify-center p-3 rounded-2xl transition-all duration-200 text-center outline-none",
                      isCurrentActive
                        ? "bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 shadow-sm"
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "h-12 w-12 rounded-2xl flex items-center justify-center mb-2 shadow-sm transition-transform duration-200 group-hover:scale-105 group-hover:shadow-md",
                        app.color.bg,
                        app.color.text
                      )}
                    >
                    ...
                    <span
                      className={cn(
                        "text-xs font-medium truncate w-full tracking-tight",
                        isCurrentActive
