...
import { MapPin, Clock, Calendar, ClipboardList, Building2, AlertCircle, AlertTriangle, CheckCircle2, Navigation2, ArrowRight, Target, History as HistoryIcon } from 'lucide-react';
...
export const HubinStudentView: React.FC<HubinStudentViewProps> = React.memo(({
  ...
  if (!studentPkl) {
    return <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
      ...
      <TabsContent value="record" className="mt-0 space-y-4 animate-fadeIn">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="lg:col-span-5">
            {isMockLocation && <div className="mb-4 p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/50 rounded-xl flex gap-3 text-rose-600 dark:text-rose-400 animate-pulse">
            ...
            <HubinTodayPresensiCards todayAbsensi={todayAbsensi} studentPkl={studentPkl} location={location} isMockLocation={isMockLocation} kegiatan={kegiatan} checkInMutation={checkInMutation} checkOutMutation={checkOutMutation} studentName={user?.full_name} onRefreshLocation={onRefreshLocation} />

            {/* Quick Riwayat Link - Compact */}
            <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-700 dark:text-white border border-slate-100 dark:border-slate-700 shadow-sm flex items-center justify-between group cursor-pointer hover:bg-white dark:hover:bg-slate-750 transition-all" onClick={handlePrintClick}>
          ...
          <div className="lg:col-span-7">
            <HubinTimelineLogbookBuilder parsedTimeline={parsedTimeline} handleDeleteActivity={onDeleteActivity} onOpenAddModal={onOpenAddModal} todayAbsensi={todayAbsensi} />
          </div>
        </div>
      </TabsContent>

      <TabsContent value="jurnal" className="mt-0">
        <HubinStudentJurnalTab studentPkl={studentPkl as any} jurnalUrl={jurnalUrl} setJurnalUrl={setJurnalUrl} submitJurnalMutation={submitJurnalMutation} onPrint={onPrint} rawAbsensiHistory={rawAbsensiHistory} />
      </TabsContent>

      <TabsContent value="portofolio" className="mt-0">
        <SectionCard title="Laporan & Portofolio Akhir" icon={ClipboardList}>
          <div className="space-y-6 p-4">
            <div className="p-6 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">
              <HubinGoogleDriveUploader label="File Laporan / Portofolio (PDF/Link Drive)" value={jurnalUrl} onChange={setJurnalUrl} studentEmail={user?.email} customFileName={generateCustomFileName('Laporan_Akhir.pdf')} folderName={studentPkl?.Siswa?.Kelas?.nama} />
              
              <div className="mt-6 flex justify-end">
                <Button variant="primary" onClick={handleSubmitPortofolio} isLoading={submitJurnalMutation.isPending} disabled={!jurnalUrl || submitJurnalMutation.isPending} className="px-8 font-black uppercase tracking-widest text-xs">
                  Kumpulkan Sekarang
                </Button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="p-5 bg-emerald-50 dark:bg-emerald-950/20 rounded-xl border border-emerald-100 dark:border-emerald-900/30 flex flex-col justify-between gap-3">
                ...
                {studentPkl?.nilai_akhir && <Button size="sm" variant="outline" className="w-full text-[10px] h-8 font-black uppercase tracking-wider border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:text-emerald-400 dark:border-emerald-900/30" onClick={() => {
