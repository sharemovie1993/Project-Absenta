/**
 * SiswaDocsPanel.tsx
 * Panel berkas kependidikan siswa dengan sistem slot kategori visual wajib
...
// Daftar Kategori Berkas Wajib untuk Siswa
...
export const SiswaDocsPanel: React.FC<SiswaDocsPanelProps> = React.memo(({
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['siswa-docs', siswaId],
    queryFn:  () => listSiswaDocuments(siswaId),
    enabled:  !!siswaId,
  });
  ...
  const handleDelete = useCallback(async (doc: MemberDoc) => {
    ...
    if (!ok) return;
    try {
      await deleteSiswaDocument(siswaId, doc.id);
      toast.success('Berkas berhasil dihapus');
      if (selectedDoc?.id === doc.id) setSelectedDoc(null);
      queryClient.invalidateQueries({ queryKey: ['siswa-docs', siswaId] });
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : 'Gagal menghapus berkas');
    }
  }, [siswaId, selectedDoc, confirm, queryClient]);
  ...
  const handleUploadSlot = async (category: MemberDocKategori, file: File) => {
    ...
    // Auto-compose judul berkas
    const nisOrNisn = nis ? nis : (nisn ? nisn : '');
    ...
    if (MULTI_CATEGORIES.includes(category)) {
      ...
      autoTitle = `${KATEGORI_LABELS[category]} - ${cleanFileName} - ${labelPrefix}${siswaName || 'Siswa'}`;
    } else {
      autoTitle = `${KATEGORI_LABELS[category]} - ${labelPrefix}${siswaName || 'Siswa'}`;
    }
  ...
  const renderSlot = (category: MemberDocKategori) => {
    ...
    // Kategori Single-File (KTP, KK, AKTA, FOTO, dll)
                ...
                <form onSubmit={handleUploadOther} className="p-3 rounded-2xl border border-dashed border-indigo-200 bg-indigo-50/10 dark:border-indigo-900/40 dark:bg-indigo-950/10 space-y-2.5">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="cth. Sertifikat Lomba Menggambar"
