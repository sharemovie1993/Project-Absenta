...
export const DashboardHero: React.FC<DashboardHeroProps> = ({
  ...
  return (
    ...
    >
      {/* Abstract Background Shapes for Wow Factor */}
      ...
      <div className={`absolute right-40 -bottom-20 ${compact ? 'w-48 h-48' : 'w-64 h-64'} rounded-full bg-indigo-400 opacity-20 blur-2xl`}></div>
      <div className="absolute left-1/2 top-0 w-32 h-32 rounded-full bg-blue-300 opacity-10 blur-xl"></div>
      ...
      <div className={`relative z-10 flex flex-col lg:flex-row ${compact ? 'lg:items-end' : 'lg:items-center'} justify-between gap-6`}>
        <div className={`flex-1 ${compact ? 'space-y-3' : 'space-y-4'}`}>
          {badge && (
            <div className={`inline-flex items-center gap-2 backdrop-blur-md border border-white/20 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider ${badgeColors[badge.color || 'blue']}`}>
          ...
          <div>
            ...
            <p className={`text-blue-100 ${compact ? 'text-sm md:text-base lg:text-lg' : 'text-base md:text-lg lg:text-xl'} font-medium max-w-2xl opacity-90 leading-relaxed`}>
              {subtitle}
            </p>
          </div>

          {currentDate && (
            <div className={`text-[10px] md:text-xs font-semibold text-blue-200/80 bg-black/10 w-fit px-3 py-1 rounded-lg backdrop-blur-sm`}>
              {currentDate}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex lg:flex-wrap gap-3 md:gap-4 self-start lg:self-center w-full lg:w-auto">
            {stats.map((stat, idx) => (
              ...
              >
                ...
                <div className="text-[8px] md:text-[10px] text-blue-100 mt-1 font-bold uppercase tracking-widest opacity-80">
                  {stat.label}
                </div>
              </motion.div>
            ))}
