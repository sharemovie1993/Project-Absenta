...
const DEFAULT_PRESETS = [
  { id: '1', nama_pelanggaran: 'Keterlambatan Masuk Sekolah', poin: 10, kategori: 'Kedisiplinan' },
  { id: '2', nama_pelanggaran: 'Atribut Seragam Tidak Lengkap', poin: 5, kategori: 'Kerapian' },
  { id: '3', nama_pelanggaran: 'Penggunaan HP Saat KBM Tanpa Izin', poin: 15, kategori: 'Akademik' },
  { id: '4', nama_pelanggaran: 'Membolos Jam Pelajaran', poin: 25, kategori: 'Kedisiplinan' },
  { id: '5', nama_pelanggaran: 'Tidak Mengerjakan Tugas / Kewajiban', poin: 10, kategori: 'Akademik' },
  { id: '6', nama_pelanggaran: 'Merusak Fasilitas / Sarana Kelas', poin: 35, kategori: 'Ketertiban' },
];
...
export const AddIncidentModal: React.FC<AddIncidentModalProps> = ({
  ...
  const { data: jenisPelanggaranRes } = useQuery({
    queryKey: ['kesiswaan', 'jenis-pelanggaran'],
    queryFn: () => kesiswaanApi.getJenisPelanggaran().catch(() => ({ success: true, data: [] })),
    enabled: isOpen,
    staleTime: 5 * 60 * 1000,
  });

  const masterList: JenisPelanggaran[] = React.useMemo(() => {
    const raw = Array.isArray(jenisPelanggaranRes?.data)
      ? jenisPelanggaranRes.data
      : Array.isArray(jenisPelanggaranRes)
        ? jenisPelanggaranRes
        : [];
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-slate-800 dark:text-slate-200">
          <div>
            ...
            <select
              ...
              className="w-full text-xs p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-800"
            >
              {students.map(s => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full text-xs p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-800"
              >
                {masterList.map((j) => (
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="w-2/3 text-xs p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-800"
                >
                ...
                <input
                  ...
                  className="w-1/3 text-xs p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-center font-bold bg-white dark:bg-slate-800"
                />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full text-xs p-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-800"
              required
            />
          </div>
