        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 text-white rounded-2xl p-6 shadow-md border border-slate-800 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-300">
              Disiplin Rombel: <strong className="text-emerald-400">{beratcount === 0 ? 'Sangat Baik' : 'Perlu Pengawasan'}</strong> {beratcount === 0 ? '(Tidak ada kasus berat berturut-turut).' : `(${beratcount} kasus berat).`}
            </p>
          </div>

          <div className="mt-4 pt-4 border-t border-slate-700/80 flex items-center justify-between text-xs text-slate-300">
            <span>Rata-rata / siswa: {(totalPoints / Math.max(totalStudents, 1)).toFixed(1)} Poin</span>
            ...
            <div className="mb-2">
              {!isApiConnected ? (
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                  ...
                  Kedisiplinan Clean • API Terhubung
                </span>
              )}
            </div>
            <CheckCircle2 className={`w-10 h-10 ${!isApiConnected ? 'text-slate-300' : 'text-emerald-500'} mx-auto mb-2`} />
            <p className="text-sm font-bold text-slate-700">Tidak Ada Catatan Pelanggaran</p>
            <p className="text-xs text-slate-500 mt-1">
              {!isApiConnected
                ? 'Belum ada data kedisiplinan yang diterima dari server backend API.'
                : 'Siswa binaan mematuhi peraturan sekolah dengan baik.'}
            </p>
          </div>
        ) : (
