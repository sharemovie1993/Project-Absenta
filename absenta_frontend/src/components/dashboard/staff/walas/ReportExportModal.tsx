...
export const ReportExportModal: React.FC<ReportExportModalProps> = ({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-4xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-slate-200 dark:border-slate-800 relative flex flex-col justify-between">
        ...
        <div className="bg-slate-900 text-white p-5 rounded-t-3xl flex items-center justify-between sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handlePrint} className="inline-flex items-center gap-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs px-3.5 py-1.5 rounded-xl transition-all cursor-pointer shadow-xs">
        ...
        <div className="bg-slate-100 dark:bg-slate-800 p-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-center gap-3 text-xs font-bold">
          <button onClick={() => setActiveReportType('attendance')} className={`px-4 py-2 rounded-xl transition-all cursor-pointer ${activeReportType === 'attendance' ? 'bg-white dark:bg-slate-900 text-indigo-900 dark:text-indigo-300 shadow-xs ring-2 ring-indigo-500/20' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
            📜 1. Rekapitulasi Presensi Kehadiran Bulanan
          </button>
          <button onClick={() => setActiveReportType('character')} className={`px-4 py-2 rounded-xl transition-all cursor-pointer ${activeReportType === 'character' ? 'bg-white dark:bg-slate-900 text-indigo-900 dark:text-indigo-300 shadow-xs ring-2 ring-indigo-500/20' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
        ...
        <div className="p-8 space-y-6 print:p-0 font-serif text-slate-900 bg-white">
          ...
          <div className="border-b-4 border-double border-slate-900 pb-4 text-center font-serif relative">
            <div className="flex items-center justify-center gap-3">
              {tenant?.logo_url ? <img src={tenant.logo_url} alt={schoolName} className="w-12 h-12 object-contain" /> : <School className="w-10 h-10 text-slate-900" />}
              <div>
          ...
          {activeReportType === 'attendance' ? <div>
              ...
              <table className="w-full text-xs text-left border-collapse border border-slate-400 font-sans">
                ...
                <tbody>
                  {students.map((s, idx) => <tr key={s.id} className="border-b border-slate-300">
              ...
              <table className="w-full text-xs text-left border-collapse border border-slate-400 font-sans">
                ...
                <tbody>
                  {students.slice(0, 15).map((s, idx) => <tr key={s.id} className="border-b border-slate-300">
                      ...
                      <td className="p-2 border border-slate-300 text-[11px] italic">
                        {s.attendanceRate === 100 ? 'Siswa sangat disiplin, aktif, dan menjadi contoh teladan di kelas.' : s.attendanceRate >= 85 ? 'Proaktif mengikuti KBM, komunikasi orang tua berjalan lancar.' : 'Perlu pengawasan presensi dan koordinasi berkala dengan orang tua.'}
                      </td>
                    </tr>)}
                </tbody>
              </table>
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-b-3xl border-t border-slate-200 dark:border-slate-700 flex justify-end">
          <button onClick={onClose} className="px-5 py-2 bg-slate-900 text-white font-bold text-xs rounded-xl hover:bg-slate-800 cursor-pointer">
