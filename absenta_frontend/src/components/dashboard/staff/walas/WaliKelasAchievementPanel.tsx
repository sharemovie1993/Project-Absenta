              ...
              <div className="mb-2">
                {!isApiConnected ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-800 border border-purple-300">
                    ...
                    API Terhubung • 0 Siswa Teladan
                  </span>
                )}
              </div>
              <Trophy className="w-8 h-8 text-slate-300 mx-auto mb-2" />
              <p className="text-sm font-bold text-slate-800">Belum Ada Data Siswa Teladan</p>
              <p className="text-xs text-slate-500 mt-1">
                {!isApiConnected
                  ? 'Gagal terhubung ke server backend API.'
                  : 'Siswa teladan akan dikalkulasi secara otomatis dari data presensi & akumulasi poin kebaikan.'}
              </p>
            </div>
          ) : (
          ...
          {achievements.length === 0 ? (
            <div className="col-span-full p-6 text-center border border-dashed border-slate-200 rounded-2xl">
              <div className="mb-2">
                {!isApiConnected ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                    ...
                    API Terhubung • 0 Catatan Prestasi
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                {!isApiConnected
                  ? 'Gagal terhubung ke server backend API.'
                  : 'Poin apresiasi dan piagam kejuaraan siswa binaan belum terdaftar di database.'}
              </p>
            </div>
          ) : (
