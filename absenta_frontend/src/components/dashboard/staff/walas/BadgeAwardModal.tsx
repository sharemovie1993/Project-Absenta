...
export const BadgeAwardModal: React.FC<BadgeAwardModalProps> = ({
  ...
  const BADGE_PRESETS = [
    { name: '🌟 Bintang Kehadiran', icon: 'Star', category: 'Kedisiplinan', desc: 'Diberikan untuk persentase kehadiran 100% atau sangat konsisten.' },
    { name: '🏆 Pejuang Prestasi', icon: 'Trophy', category: 'Prestasi', desc: 'Diberikan atas capaian juara perlombaan/kegiatan akademik/non-akademik.' },
    { name: '💎 Teladan Karakter', icon: 'Shield', category: 'Karakter', desc: 'Diberikan untuk kepribadian santun, jujur, dan aktif membantu teman.' },
    { name: '🚀 Siswa Paling Disiplin', icon: 'Zap', category: 'Kedisiplinan', desc: 'Diberikan atas ketaatan aturan sekolah dan seragam lengkap.' }
  ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAwardBadge(
      ...
      selectedPreset.name,
      selectedPreset.icon,
      customNote || `Apresiasi atas dedikasi ${student.name} sebagai siswa teladan XI RPL 1.`
    );
    onClose();
  };
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-slate-200 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {BADGE_PRESETS.map((b) => (
                <div
                  key={b.name}
                  onClick={() => setSelectedPreset(b)}
                  className={`p-3 rounded-2xl border transition-all cursor-pointer ${
                    selectedPreset.name === b.name
                      ? 'border-purple-600 bg-purple-50 ring-2 ring-purple-500/20 shadow-xs'
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder={`Contoh: Terus pertahankan semangat belajarmu ${student.name.split(' ')[0]}, kamu adalah kebanggaan kelas!...`}
              value={customNote}
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-end gap-2">
            ...
            <button
              type="submit"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer active:scale-95"
            >
