...
export const HomeVisitSection: React.FC = React.memo(() => {
  ...
  const renderMobileCard = (item: HomeVisit & { Dokumen?: { file_original_name?: string } }) => {
    return (
      <div
        key={item.id}
      ...
      >
        <div className="flex items-start justify-between gap-2">
          ...
          {item.Dokumen && (
            <span className="flex items-center text-[10px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/40 px-2 py-1 rounded-lg">
              <Paperclip className="w-3 h-3 mr-1" />
              Lampiran
            </span>
          )}
        ...
        <div className="p-2.5 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
          <span className="text-[10px] font-bold text-slate-400 uppercase block">Alasan Kunjungan:</span>
    ...
    <Card className="border border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-900/50 p-6 rounded-2xl">
      ...
      {loading && data.length === 0 ? (
        ...
        <div className="space-y-4">
          <MobileAcademicList
            title="Daftar Kunjungan Rumah"
            data={data}
            loading={loading}
            totalItems={totalPages * limit}
            emptyMessage="Belum ada catatan kunjungan rumah (home visit)."
            pagination={{
              currentPage: page,
              itemsPerPage: limit,
              totalItems: totalPages * limit,
