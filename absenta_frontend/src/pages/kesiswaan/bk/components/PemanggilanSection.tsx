...
import { bkApi, type PemanggilanOrangTua } from '../../../../api/kesiswaan/bk.api';
...
export const PemanggilanSection: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await bkApi.getPemanggilan({
        page,
        limit,
  ...
  const handleEdit = (item: PemanggilanOrangTua) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await bkApi.deletePemanggilan(id);
      if (res.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await bkApi.createPemanggilan(formData);
      success('Surat pemanggilan orang tua berhasil dibuat');
      setModalOpen(false);
      resetForm();
      fetchData();
    } catch (err: any) {
      error(err.message || 'Gagal menyimpan pemanggilan');
    }
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await bkApi.updatePemanggilan(selectedId, {
      ...
      error(err.message || 'Gagal memperbarui pemanggilan');
    } finally {
  ...
  return (
    <Card className="border border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-900/50 p-6 rounded-2xl">
      ...
      <Suspense fallback={null}>
        <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Buat Pemanggilan Orang Tua Baru" size="lg">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Tanggal Rencana Pemanggilan</Label>
              <Input
                type="date"
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Alasan Pemanggilan</Label>
              <textarea
      ...
      <Suspense fallback={null}>
        <Modal isOpen={editModalOpen} onClose={() => setEditModalOpen(false)} title="Perbarui Hasil Pertemuan Wali Murid" size="lg">
          <form onSubmit={handleEditSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Status Kehadiran Orang Tua</Label>
              <SearchableSelect
                options={[
            ...
            {editFormData.status === 'HADIR' && (
              <>
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Tanggal Pertemuan</Label>
                  <Input
                    type="date"
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Hasil Pertemuan / Berita Acara</Label>
                  <textarea
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Unggah Bukti Hasil Scan Berita Acara / Surat Perjanjian (Opsional)</Label>
                  <Input
                    type="file"
