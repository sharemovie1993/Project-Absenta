...
import { bkApi, type JenisPrestasi } from '../../../../api/kesiswaan/bk.api';
...
export const SettingsSection: React.FC = () => {
  ...
  const fetchAchievements = useCallback(async () => {
    try {
      const res = await bkApi.getJenisPrestasi();
  ...
  const handleVDelete = async (id: string) => {
  ...
  const handleASubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (aId) {
        await bkApi.updateJenisPrestasi(aId, aForm);
        success('Kategori prestasi berhasil diperbarui');
      } else {
        await bkApi.createJenisPrestasi(aForm);
  ...
  const handleADelete = async (id: string) => {
    ...
    if (!ok) return;
    try {
      await bkApi.deleteJenisPrestasi(id);
  ...
  const achievementColumns: Column[] = [
    {
      key: 'kategori',
      label: 'Kategori',
      render: (value: string) => (
    ...
    <Card className="border border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-900/50 p-6 rounded-2xl">
      ...
      <Modal isOpen={violationModalOpen} onClose={() => setViolationModalOpen(false)} title={vId ? 'Edit Kategori Pelanggaran' : 'Tambah Kategori Pelanggaran Baru'} size="sm">
        <form onSubmit={handleVSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Tingkat Pelanggaran</Label>
            <SearchableSelect
              options={[
              ...
              onChange={(val) => setVForm(prev => ({ ...prev, kategori: val }))}
              className="h-10 text-xs border-slate-200/60 dark:border-slate-800 rounded-xl"
            />
          </div>
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Jenis / Nama Pelanggaran</Label>
            <Input
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Bobot Poin Pelanggaran</Label>
            <Input
              type="number"
              min="1"
              value={vForm.poin || ''}
      ...
      <Modal isOpen={achievementModalOpen} onClose={() => setAchievementModalOpen(false)} title={aId ? 'Edit Kategori Prestasi' : 'Tambah Kategori Prestasi Baru'} size="sm">
        <form onSubmit={handleASubmit} className="space-y-4">
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Kategori Kegiatan</Label>
            <SearchableSelect
              options={[
              ...
              onChange={(val) => setAForm(prev => ({ ...prev, kategori: val }))}
              className="h-10 text-xs border-slate-200/60 dark:border-slate-800 rounded-xl"
            />
          </div>
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Nama / Kategori Penghargaan</Label>
            <Input
