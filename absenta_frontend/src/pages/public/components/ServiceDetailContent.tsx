...
export function ServiceDetailContent() {
  ...
  const subQuery = useQuery<MySubscriptionData | null>({
    queryKey: ['my-subscription', tenantId],
    queryFn: async () => {
      const res = await getMySubscription();
      return res.success ? (res.data as MySubscriptionData) : null;
    },
  ...
  const product = useMemo(() => {
    ...
    const matched = list.find(g => 
      ...
      g.variants.some((v: Plan) => v.id?.toUpperCase() === slugUpper)
    );
  ...
  return (
    <div className="pb-12 max-w-7xl mx-auto w-full min-w-0 max-w-full">
      {/* Breadcrumb - Compact */}
      ...
      <SectionCard noPadding fullWidth className="mb-12 border-none shadow-2xl rounded-[2.5rem] overflow-hidden w-full min-w-0 max-w-full">
        <div className="p-8 md:p-12 bg-white dark:bg-slate-900 relative w-full min-w-0 max-w-full">
          <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 blur-[100px] -mr-32 -mt-32" />
          
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 relative z-10 w-full min-w-0 max-w-full">
            ...
            <div className="space-y-10 w-full min-w-0 max-w-full">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {[
                      { id: 'MONTH' as const, label: 'Bulanan', icon: <Smartphone size={16} /> },
                      { id: 'YEAR' as const, label: 'Tahunan', icon: <Zap size={16} /> }
                    ]?.map((p) => {
            ...
            <div className="lg:border-l lg:border-slate-100 lg:dark:border-slate-800 lg:pl-20 flex flex-col justify-center w-full min-w-0 max-w-full">
      ...
      <SectionCard noPadding fullWidth className="mb-12 border-none shadow-2xl rounded-[3rem] overflow-hidden w-full min-w-0 max-w-full">
        <div className="p-10 bg-gradient-to-br from-slate-900 to-blue-900 text-white relative group w-full min-w-0 max-w-full">
          ...
          <Button type="button" className="w-full mt-10 rounded-2xl bg-white text-blue-950 font-black h-14 text-[11px] uppercase tracking-[0.2em] hover:bg-blue-50 transition-all shadow-xl active:scale-[0.98]">
      ...
      <SectionCard noPadding fullWidth className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden group w-full min-w-0 max-w-full">
        <div className={`absolute inset-0 bg-gradient-to-b ${theme.gradient} opacity-20`}></div>
        <div className="p-12 relative z-10 flex flex-col md:flex-row items-center gap-12 w-full min-w-0 max-w-full">
          ...
          <div className="md:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-5 relative z-10 w-full min-w-0 max-w-full">
