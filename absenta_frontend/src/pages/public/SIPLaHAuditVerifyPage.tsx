...
export default function SIPLaHAuditVerifyPage() {
  ...
  const handleVerifySearch = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setAuditResult({
        ...
        warrantyExpireDate: '29 Juli 2029',
        auditHash: `BTI-AUDIT-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
        verifiedAt: new Date().toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })
      });
      setLoading(false);
    }, 600);
  };
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto space-y-8 pb-16">
        ...
        <Card className="p-8 bg-slate-900 text-white rounded-3xl border-slate-800 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative space-y-6">
            ...
            <form onSubmit={handleVerifySearch} className="flex flex-col sm:flex-row gap-3">
              ...
              <Button 
                type="submit" 
                variant="primary" 
                className="py-3.5 px-8 bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-widest rounded-2xl shrink-0"
              >
                {loading ? <Loader size="sm" /> : 'Periksa Verifikasi'}
              </Button>
            </form>
          </div>
        </Card>
        ...
        {searched && auditResult && (
          <Card className="p-8 md:p-12 bg-white dark:bg-slate-900 border-2 border-emerald-500/30 rounded-3xl shadow-xl relative space-y-8 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="p-5 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 text-xs text-blue-900 dark:text-blue-200 leading-relaxed flex items-start gap-3">
