...
import { z } from 'zod';
import { SectionCard, Button, Input, Badge, Loader, Alert, AlertDescription, Table } from '../../../components/ui';
...
export default React.memo(function RekapHarianSiswaPage() {
  ...
  const rekapQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await getRekapHarianSiswa(siswaId, {
        tanggal,
  ...
  const fetchData = useCallback(async () => {
    const parseResult = rekapHarianFormSchema.safeParse({
      siswaId,
      tanggal,
      tahunPelajaranId
    });
  ...
  const columns = useMemo(() => [{
    ...
    key: 'waktu',
    sortable: true,
    render: (v: unknown) => <div className="flex items-center gap-2">
    ...
    render: (v: unknown, r: unknown) => {
      ...
      return <span className="text-[11px] font-bold uppercase tracking-tight text-slate-500">
    ...
    render: (v: unknown) => {
      ...
      return <Badge variant={isHadir ? 'success' : isPulang ? 'info' : 'outline'} className="text-[10px] font-black uppercase tracking-widest px-3">
            {statusStr || '-'}
          </Badge>;
    }
  }], []);
  ...
  const breadcrumbs = useMemo(() => [{
    ...
    label: 'Harian Siswa',
    path: '/attendance/rekap/siswa-harian'
  }], []);
  if (loading) {
  ...
  const pageContent = <div className="space-y-6">
      <SectionCard title="Filter Laporan Harian" icon={Filter} fullWidth>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
          <div className="space-y-2">
            ...
            <SearchableSelect id="select-rekap-siswa" aria-label="Pilih Siswa" value={siswaId} onValueChange={setSiswaId} options={siswaOptions} placeholder="Cari Nama Siswa..." searchPlaceholder="Ketik nama siswa..." triggerClassName="h-12 rounded-xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 font-bold" disabled={isSiswa} />
          </div>
          <div className="space-y-2">
            ...
            <Input id="input-rekap-tanggal" aria-label="Tanggal Laporan" type="date" value={tanggal} onChange={e => setTanggal(e.target.value)} className="h-12 rounded-xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 font-bold" />
          </div>
          <div className="space-y-2">
            ...
            <SearchableSelect id="select-rekap-tahun" aria-label="Tahun Pelajaran" value={tahunPelajaranId} onValueChange={setTahunPelajaranId} options={tahunOptions} placeholder="Pilih Tahun..." triggerClassName="h-12 rounded-xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 font-bold" />
          </div>
        </div>
        <div className="mt-8 flex justify-end">
          <Button onClick={fetchData} disabled={loading || !siswaId || !tanggal} className="h-12 px-10 rounded-xl font-black text-[11px] uppercase tracking-widest gap-2 bg-slate-900 dark:bg-blue-600 text-white shadow-xl hover:scale-[1.02] active:scale-95 transition-all">
      ...
      {data && <SectionCard title="Hasil Rekapitulasi Harian" icon={FileText} fullWidth noPadding>
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-tight">
                      {formatDate(String(data.tanggal), {
                    weekday: 'long',
                    day: '2-digit',
                    month: 'long',
                    year: 'numeric',
              ...
              <div className="flex flex-col items-end">
                 ...
                 <Badge variant={data.status?.toUpperCase().includes('HADIR') && !data.status?.toUpperCase().includes('BELUM') ? 'success' : 'secondary'} className="h-10 px-6 rounded-xl text-[11px] font-black uppercase tracking-[0.15em] shadow-sm">
  ...
  return <AcademicPageLayout hardeningModuleKey="rekaphariansiswapage" title="Rekap Harian Siswa" description="Rincian kehadiran harian siswa beserta status dan waktu pencatatan" breadcrumbs={breadcrumbs} instruction={{
    ...
    items: [{
      ...
      <Suspense fallback={<div className="flex justify-center p-8"><Loader size="lg" /></div>}>
        <PremiumFeatureGate isLocked={isLocked} moduleName="ABSENSI" featureName="Rekap Presensi Harian" description="Dapatkan rincian kehadiran harian siswa secara mendalam, lengkap dengan log waktu dan status kegiatan.">
          {pageContent}
        </PremiumFeatureGate>
      </Suspense>
    </AcademicPageLayout>;
