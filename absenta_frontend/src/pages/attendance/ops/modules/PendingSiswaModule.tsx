...
export const PendingSiswaModule: React.FC<PendingSiswaModuleProps> = ({
  notPresent,
  notPresentLoading,
  miniStats,
  selectedKelasId,
  ...
  const inferredKelasId = useMemo(() => {
    ...
    const inferred =
      notPresent.find((x: any) => x?.kelas_id)?.kelas_id ??
      notPresent.find((x: any) => x?.kelasId)?.kelasId ??
      '';
  ...
  const filteredNotPresent = useMemo(() => {
    ...
    return notPresent.filter((x: any) => {
  ...
  const handleMarkStatus = async (siswaId: string, status: string) => {
    try {
      await markGateAbsence({ siswa_id: siswaId, status: status as any });
      ...
      const msg = e?.response?.data?.message || e?.message || 'Gagal merekam status';
      if (statusCode === 409) {
        // Sudah terekam → tetap refresh agar siswa hilang dari daftar
            ...
            <div className="mt-4 space-y-4">
               {!isPetugasSiswa && (
                  <div className="space-y-2">
                     <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-4">Filter Kelas</label>
                     <SearchableSelect
