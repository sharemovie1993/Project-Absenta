...
function CalendarCard({ 
  ...
  const { cells, statsCards } = useMemo(() => {
    ...
    const statsCards = [
      { label: 'Hadir', value: stats.HADIR ?? 0, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
      ...
      { label: 'Alpa', value: stats.ALPA ?? 0, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-950/20' },
    ...
    async function initData() {
      try {
        ...
        if (kelasId) {
           ...
           siswaOpts = (res.data || []).map((s: any) => ({ value: s.id, label: `${s.nama_siswa} - ${s.nis}` }));
        } else {
            ...
            <div className="bg-white dark:bg-slate-950 overflow-hidden">
               {result && result.kegiatan && result.kegiatan.length > 0 ? (
                 <div className="divide-y divide-slate-50 dark:divide-slate-900">
                    {result.kegiatan.map((item: any, i: number) => (
