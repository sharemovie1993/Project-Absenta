...
export const generateGenericPdf = async (options: GenerateGenericPdfOptions): Promise<Blob> => {
  ...
  // 1. Draw Kop Surat Header (Unified Official look)
  if (logoDaerahBase64) {
    try {
  ...
  // 2. Draw Content by Module and PrintType
  ...
  if (module === 'kurikulum') {
    if (printType === 'roster') {
      ...
      doc.setFont('Helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.text(`KELAS: ${selectedClassId.toUpperCase() || 'SEMUA KELAS'}`, pageWidth / 2, headerEndY + 11, { align: 'center' });

      // Drawing Jadwal Table
      const head = [['JAM KE-', 'SENIN', 'SELASA', 'RABU', 'KAMIS', 'JUMAT']];
      const body = Array.from({ length: 8 }).map((_, i) => [
        `Jam ke-${i+1} (${7 + i}.00)`, 'Matematika', 'B. Indonesia', 'Fisika', 'B. Inggris', 'PABP'
      ]);
      autoTable(doc, {
      ...
      // General fallbacks
      doc.setFontSize(11);
      doc.text(`DOKUMEN DARING: ${printType.toUpperCase()}`, pageWidth / 2, headerEndY + 6, { align: 'center' });
    }
  ...
  // 3. Draw Shared Bottom Signature (Kepala Sekolah)
