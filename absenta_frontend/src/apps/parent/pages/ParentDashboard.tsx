...
import { Bell, ChevronRight, LogOut, Activity, UserCheck, Award, FileText, MessageCircle, CheckCircle2, Calendar, User, LayoutList, AlertTriangle, Trophy, RefreshCw, QrCode, MapPin, Heart, Phone, ArrowRight, ShieldCheck, Clock, Send, X } from 'lucide-react';
...
export default function ParentDashboard() {
  ...
  const handleSendLeaveRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newRecord = {
        ...
        badgeColor: jenisIzin === 'sakit' ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/30' : 'bg-blue-500/15 text-blue-600 dark:text-blue-400 border-blue-500/30',
  ...
  useEffect(() => {
    getParentDashboard().then(newData => setData(newData)).catch(console.error);
  }, [setData]);
  ...
  useEffect(() => {
    ...
    const handleUpdate = (payload: any) => {
      if (payload?.data?.siswa_id === student.siswa_id) {
        getParentDashboard().then(setData).catch(console.error);
        getStudentNotifications(student.siswa_id, 1, 5).then(res => setNotifications(res.data)).catch(console.error);
      }
    };
  ...
  useEffect(() => {
    ...
    getStudentNotifications(student.siswa_id, 1, 5).then(res => setNotifications(res.data)).catch(console.error).finally(() => setLoadingNotifs(false));
  }, [student?.siswa_id]);
  ...
  if (!data || !student) {
    return <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-500 space-y-3">
  ...
  const {
    status_kehadiran_hari_ini: today,
  ...
  const tabs = [{
    id: 'ringkasan',
    label: 'Feed',
    fullLabel: 'Ringkasan Feed',
    icon: Activity
  }, {
    id: 'kehadiran',
    label: 'Presensi',
    fullLabel: 'Presensi Anak',
    icon: Calendar
  }, {
    id: 'perizinan',
    label: 'Perizinan',
    fullLabel: 'Perizinan (1-Tap)',
    icon: FileText,
    badge: '2'
  }, {
    id: 'catatan',
    label: 'Poin',
    fullLabel: 'Poin & Kasus',
    icon: ShieldCheck
  }, {
    id: 'profil',
    label: 'Profil',
    fullLabel: 'Profil Ortu',
    icon: User
  }];
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 pb-24 lg:pb-8">
      {/* ────────────────────────────────────────────────────────────────── */}
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 px-4 sm:px-8 py-3.5 sticky top-0 z-30 flex items-center justify-between shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {data.siswa.length > 1 ? <div className="w-[180px] sm:w-[220px] mt-0.5">
                <SearchableSelect value={selectedStudentId || ''} onValueChange={val => setSelectedStudentId(val)} options={data.siswa.map((s: any) => ({
              label: s.nama_siswa,
              value: s.siswa_id
            }))} placeholder="Pilih Anak" searchPlaceholder="Cari nama anak..." triggerClassName="w-full font-bold text-xs h-8" />
              </div> : <h1 className="font-extrabold text-sm sm:text-base text-slate-900 dark:text-white leading-tight">
                {student.nama_siswa}
              </h1>}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleRefresh} disabled={isRefreshing} className="p-2 rounded-xl text-slate-500 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 transition-colors" title="Refresh Data">
        ...
        }} className="p-2 rounded-xl text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors cursor-pointer" title="Keluar / Logout">
            <LogOut size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 space-y-4 sm:space-y-5">
        ...
        <div className="p-5 sm:p-7 rounded-3xl bg-gradient-to-r from-orange-600 via-amber-600 to-rose-600 text-white shadow-xl space-y-6 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 relative z-10">
            ...
            <div className="p-2.5 px-3.5 rounded-2xl bg-black/20 backdrop-blur-md border border-white/20 flex items-center gap-3 shrink-0 self-start md:self-auto shadow-inner">
              <div className="w-11 h-11 rounded-xl bg-white/20 text-white font-black text-sm flex items-center justify-center border border-white/30 shadow-xs overflow-hidden">
                {student.foto ? <img src={student.foto} alt={student.nama_siswa} className="w-full h-full object-cover" /> : <span className="font-black text-xs">{childInitials}</span>}
              </div>
              <div>
          ...
          <div className="p-1.5 rounded-2xl bg-slate-950/80 backdrop-blur-md border border-white/10 flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar relative z-10 shadow-lg">
            ...
            return <button key={tab.id} type="button" onClick={() => handleTabChange(tab.id)} className={cn("px-4 py-2.5 rounded-xl text-xs font-extrabold flex items-center gap-2 transition-all cursor-pointer whitespace-nowrap select-none", isTabActive ? "bg-slate-900 text-emerald-400 border border-emerald-500/40 shadow-md shadow-emerald-950/50" : "text-slate-300 hover:text-white hover:bg-white/10")}>
                  <TabIcon size={15} className={isTabActive ? "text-emerald-400" : "text-slate-400"} />
                  <span>{tab.fullLabel}</span>
                  {tab.badge && <span className="w-4 h-4 rounded-full bg-purple-500 text-white text-[9px] font-black flex items-center justify-center shadow-xs">
                      {tab.badge}
                    </span>}
                </button>;
          })}
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'ringkasan' && <motion.div key="tab-ringkasan" initial={{
          opacity: 0,
          y: 8
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
                ...
                <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    <div className="text-lg sm:text-xl font-black text-emerald-600 dark:text-emerald-400 uppercase">
                      {statusLabel} {today.waktu_masuk ? `(${new Date(today.waktu_masuk).toLocaleTimeString('id-ID', {
                    hour: '2-digit',
                    minute: '2-digit'
                  })} WIB)` : ''}
                    </div>
              ...
              <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
                ...
                time: today.waktu_masuk ? new Date(today.waktu_masuk).toLocaleTimeString('id-ID', {
                  hour: '2-digit',
                  minute: '2-digit'
                }) + ' WIB' : '06:48 WIB'
              }, {
                ...
                time: '06:15 WIB'
              }].map(item => <div key={item.id} className="p-4 sm:p-5 rounded-2xl bg-slate-50/90 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-slate-300 dark:hover:border-slate-700 transition-all">
          ...
          {activeTab === 'kehadiran' && <motion.div key="tab-kehadiran" initial={{
          opacity: 0,
          y: 8
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
              ...
              tapMasuk: '-',
              tapKeluar: '-'
            }].map((item, idx) => <div key={idx} className="p-4 sm:p-5 rounded-2xl bg-slate-50/90 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 hover:border-slate-300 dark:hover:border-slate-700 transition-all">
          ...
          {activeTab === 'perizinan' && <motion.div key="tab-perizinan" initial={{
          opacity: 0,
          y: 8
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
                ...
                <form onSubmit={handleSendLeaveRequest} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      <button type="button" onClick={() => setJenisIzin('sakit')} className={cn("py-2.5 rounded-xl text-xs font-extrabold border transition-all cursor-pointer", jenisIzin === 'sakit' ? "bg-amber-600 text-white border-amber-600 shadow-md shadow-amber-600/20" : "bg-slate-50 dark:bg-slate-800/60 text-slate-600 dark:text-slate-300 border-slate-200/80 dark:border-slate-700 hover:bg-slate-100")}>
                        🩺 Sakit
                      </button>
                      <button type="button" onClick={() => setJenisIzin('izin')} className={cn("py-2.5 rounded-xl text-xs font-extrabold border transition-all cursor-pointer", jenisIzin === 'izin' ? "bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-600/20" : "bg-slate-50 dark:bg-slate-800/60 text-slate-600 dark:text-slate-300 border-slate-200/80 dark:border-slate-700 hover:bg-slate-100")}>
                        🏠 Izin
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea rows={3} value={alasanIzin} onChange={e => setAlasanIzin(e.target.value)} placeholder="Contoh: Ananda sakit demam tinggi sejak semalam, saran dokter istirahat total 2 hari." className="w-full p-3.5 rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/70 text-xs font-medium focus:outline-none focus:border-amber-500 transition-all placeholder:text-slate-400" />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <label className="p-4 sm:p-5 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-950/40 text-center hover:border-amber-500/50 transition-all cursor-pointer block group">
                      <input type="file" accept="image/*,application/pdf" onChange={e => setAttachmentFile(e.target.files?.[0] || null)} className="hidden" />
                  ...
                  {/* Submit Button */}
                  <Button type="submit" disabled={isSubmittingIzin} className="w-full h-11 rounded-2xl text-xs font-extrabold bg-amber-600 hover:bg-amber-700 text-white border-none flex items-center justify-center gap-2 shadow-lg shadow-amber-600/20 cursor-pointer disabled:opacity-50">
                    {isSubmittingIzin ? <RefreshCw size={15} className="animate-spin" /> : <Send size={15} />}
              ...
              <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-5">
                ...
                <div className="space-y-3">
                  {leaveHistory.map(item => <div key={item.id} className="p-4 sm:p-5 rounded-2xl bg-slate-50/90 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 space-y-3 hover:border-slate-300 dark:hover:border-slate-700 transition-all">
          ...
          {activeTab === 'catatan' && <motion.div key="tab-catatan" initial={{
          opacity: 0,
          y: 8
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
              ...
              poin: '-25 Poin',
              poinColor: 'text-rose-600 dark:text-rose-400'
            }].map(item => <div key={item.id} className="p-4 sm:p-5 rounded-2xl bg-slate-50/90 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 flex items-center justify-between gap-4 hover:border-slate-300 dark:hover:border-slate-700 transition-all">
                    <div className="space-y-1 min-w-0">
                      <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white leading-snug">
                        {item.title}
                      </h4>
          ...
          {activeTab === 'profil' && <motion.div key="tab-profil" initial={{
          opacity: 0,
          y: 8
        }} animate={{
          opacity: 1,
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-slate-200/80 dark:border-slate-800 px-1 pt-1.5 pb-[calc(0.5rem+env(safe-area-inset-bottom))] flex justify-around items-center shadow-xl">
          ...
          const TabIcon = tab.icon;
          const isTabActive = activeTab === tab.id;
          return <button key={tab.id} type="button" onClick={() => handleTabChange(tab.id)} className={cn("flex flex-col items-center justify-center gap-0.5 p-1 rounded-xl text-[10px] font-bold transition-all duration-200 select-none flex-1 relative cursor-pointer", isTabActive ? "text-emerald-600 dark:text-emerald-400 font-black" : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200")}>
                <div className={cn("p-1.5 rounded-xl transition-all relative", isTabActive ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : "bg-transparent")}>
                  <TabIcon size={18} />
                  {tab.badge && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-purple-500 text-white text-[8px] font-black flex items-center justify-center border border-white dark:border-slate-900">
                      {tab.badge}
                    </span>}
                </div>
        ...
        <ReportAbsenceModal isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} studentId={student.siswa_id} onSuccess={() => {
        getParentDashboard().then(setData).catch(console.error);
      }} />
      </div>
    </div>;
