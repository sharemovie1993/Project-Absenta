...
export default defineConfig(({ mode }) => {
  ...
  const isRelativeApi = env.VITE_API_BASE_URL?.startsWith('/');
  const rawProxyTarget = env.VITE_PROXY_TARGET || (isRelativeApi ? 'http://localhost:3001' : env.VITE_API_BASE_URL) || 'http://localhost:3001';
  const proxyTarget = rawProxyTarget.replace('://localhost', '://127.0.0.1');
  const isTargetApi = proxyTarget.endsWith('/api');
