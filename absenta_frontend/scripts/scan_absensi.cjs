/**
 * scan_absensi.cjs — Scan semua file modul Absensi / Attendance untuk auth patterns
 */
const fs = require('fs');
...
for (const dir of SCAN_DIRS) {
  ...
  for (const file of files) {
    ...
    results.push({ rel, sizeKb, canCount, useCapsCount, useAuthCount, hardcoded, capsIncludes, score, canMatches });
  }
}

results.sort((a, b) => b.score - a.score);

console.log('\n=== SCAN MODUL ABSENSI / ATTENDANCE ===\n');
for (const r of results) {
  const flag = r.hardcoded > 0 ? '🔴' : (r.useCapsCount === 0 && r.useAuthCount > 0) ? '🟠' : r.canCount > 0 ? '🟡' : '⚪';
  if (r.canCount > 0 || r.hardcoded > 0 || r.capsIncludes > 0 || r.useAuthCount > 0) {
...
console.log('\nTotal file Absensi / Attendance:', results.length);
