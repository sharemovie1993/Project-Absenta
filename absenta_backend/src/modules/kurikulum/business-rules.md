...
### 6. Kalender Akademik (Academic Calendar)
- **Tenant Scope Isolation**: Seluruh event kalender akademik harus terikat ketat pada `tenant_id` dan `tahun_pelajaran_id` tertentu. Tidak diizinkan melakukan bypass kueri lintas tenant untuk menghindari kebocoran agenda sekolah (IDOR Protection).
- **Idempotency Auto-Seeding**: Aksi seeding massal hari libur harus idempoten. Sistem wajib memverifikasi kecocokan kriteria (kombinasi `tenant_id`, `tahun_pelajaran_id`, `judul`, dan `tanggal_mulai`) sebelum menyimpan entri baru untuk mencegah data ganda.
- **Timezone-Safe Date Comparison**: Pencocokan event kalender, hari efektif, dan filter bulan/tahun harus menggunakan konversi string aman `YYYY-MM-DD` untuk menghindari pergeseran jam akibat perbedaan zona waktu server/database (UTC vs WIB).
- **Dynamic Minggu Efektif (RPE)**: Penghitungan minggu efektif per semester (Semester 1/Ganjil & Semester 2/Genap) wajib diselaraskan dengan hari sekolah aktif (`tenant.hari_sekolah`) dan mengeliminasi hari yang terdaftar sebagai Libur Nasional, Libur Sekolah, STS (PTS), dan SAS (PAS).
- **Conflict Warning Alert**: Form pembuatan/pengeditan event wajib mendeteksi tumpang tindih tanggal antara event non-libur dengan event libur yang sudah ada, serta menampilkan pesan peringatan kepada pengguna secara real-time.

### 7. Jadwal Piket Guru & SaaS Dynamic Reminder Engine (WhatsApp Gateway)
- **Timezone-Aware Dynamic Dispatching**: Pengiriman pengingat jadwal piket guru ke Grup WhatsApp sekolah tidak menggunakan cron statis hardcoded, melainkan dievaluasi secara dinamis setiap menit (`piketScheduleDispatcher.job`) berbasis zona waktu lokal masing-masing sekolah (`tenant.timezone` seperti `Asia/Jakarta`, `Asia/Makassar`, `Asia/Jayapura`).
- **Flexible Custom Time Alignment**: Sistem mencocokkan waktu riil lokal tenant terhadap preferensi jam yang ditentukan sekolah di `PIKET_WA_NOTIF_CONFIG` (`morningTime` default 05:00 untuk pengingat hari ini, `nightTime` default 23:00 untuk pengingat besok).
- **Idempotency Guarantee & Anti-Duplicate Outbox**: Setiap pengiriman pesan tercatat di tabel `NotificationLog` dengan format kunci unik `related_id: "PIKET:${targetDateString}:${shiftKey}"`. Sistem memblokir pengiriman ulang jika pada tanggal dan shift tersebut pesan sudah berstatus `SENT`, menjamin 100% kebal terhadap duplikasi saat server restart atau clustering PM2.
- **Strict Working Day Verification**: Pengingat hanya dikirim jika tanggal target merupakan hari kerja sekolah aktif berdasarkan 3 lapisan verifikasi: (1) Konfigurasi `tenant.hari_sekolah` (5 hari vs 6 hari), (2) Kejadian Khusus / Libur Darurat (`AbsensiKejadianKhusus`), dan (3) Kalender Akademik Libur Terjadwal (`KalenderAkademik`).
- **Multi-Tenant Isolation & Sandbox Protection**: Hanya tenant aktif (`status = 'ACTIVE'`) dengan integrasi WhatsApp valid yang dapat melakukan pengiriman notifikasi. Tenant demo, kloning, atau trial terisolasi secara ketat dan dilarang mengirimkan pesan ke JID grup publik sekolah produksi.
- **Manual Trigger Bypass (Test Send)**: Tombol pengujian instan (*Test Kirim*) di antarmuka web memicu pengiriman dengan flag `forceSend: true` untuk melewati guard idempotensi harian tanpa merusak integritas log otomatisasi.
