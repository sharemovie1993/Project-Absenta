...
export async function createSiswaCommand(
  ...
  const siswa = await siswaDb.siswa.create({
    data: {
      ...
      anak_ke: input.anak_ke ? Number(input.anak_ke) : null,
      tinggi_badan: input.tinggi_badan ? Number(input.tinggi_badan) : null,
      berat_badan: input.berat_badan ? Number(input.berat_badan) : null,
      agama: input.agama || null,
      hobi: input.hobi || null,
      cita_cita: input.cita_cita || null,
      is_osis: !!input.is_osis,
      is_mpk: !!input.is_mpk,
      ekskul_1: input.ekskul_1 || null,
      ekskul_2: input.ekskul_2 || null,
