...
import { seedDefaultJenisPelanggaranForTenant } from '../jenis-pelanggaran.service';

let started = false;

export async function initKesiswaanTenantCreatedConsumer(): Promise<void> {
  if (started) return;
  started = true;
  ...
  sub.on('message', async (_channel: string, message: string) => {
    ...
    try {
      await seedDefaultJenisPelanggaranForTenant(tenantId);
    } catch {}
  });
}
