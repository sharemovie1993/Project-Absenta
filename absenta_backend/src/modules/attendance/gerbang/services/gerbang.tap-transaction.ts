...
import type { GerbangActivityLogMetadata } from '../types/gerbang.types';
...
export async function processTapTransaction(params: {
  ...
  const { input, siswa, userId, tenantId, tenantMode, sessionInfo, tapTime } = params;
  ...
  const tapTxResult = await gerbangDb.$transaction(async (tx) => {
    ...
    const kelasNama = siswa.Kelas?.nama_kelas || null;
    const tingkatData = kelasId
  ...
  if (isNew) {
    void (async () => {
        ...
        if (input.arah === JenisTap.GERBANG_DATANG) {
          if (isLate) {
            await emitDomainEvent({
              ...
              payload: {
                ...
                notification_hint: 'STUDENT_LATE',
              },
            });
          } else {
            await emitDomainEvent({
          ...
          await emitDomainEvent({
            ...
            payload: {
              ...
              status: 'PULANG',
              arah: input.arah,
              notification_hint: 'STUDENT_RETURN',
            },
          });
        }
      } catch (error) {
      ...
      try {
        const activityMetadata: GerbangActivityLogMetadata = {
          description: `Tap gerbang ${input.arah} untuk siswa ${siswa.nama_siswa} (${tenantMode} mode)`,
          arah: input.arah,
          siswa_id: siswa.id,
    ...
    if (String(process.env.LOG_LEVEL || '').toLowerCase() === 'debug') {
      console.log(`[GerbangService] Duplicate tap handled via upsert for siswa ${siswa.id}`);
    }
  }

  if (['SAKIT', 'IZIN', 'ALPA', 'DISPEN'].includes((absenRecord as any).status)) {
