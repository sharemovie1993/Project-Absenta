...
export async function authRoutes(fastify: any) {
  ...
  fastify.post('/register', {
    preHandler: [], // Skip auth middleware for registration
  ...
  fastify.post('/login', {
    preHandler: [], // Skip auth middleware for login
    config: {
      skipAuth: true,
      public: true,
      rateLimit: {
  ...
  fastify.post('/refresh', {
    preHandler: [], // Skip auth middleware for refresh
    config: { skipAuth: true, public: true },
  ...
  fastify.post('/impersonate', {
    preHandler: [requireCapability('superadmin.tenants.impersonate')],
    handler: authController.impersonate.bind(authController),
  });

  // Logout - requires authentication
  fastify.post('/logout', {
    preHandler: [requireCapability('core.auth.logout')],
